...
ho_{physical} / 3 c^2]$ */
  {
    ...
    double H_lcdm = sqrt(rho_tot-pba->K/a/a);
    
    /* OCTH modification: H_OCTH = H_LCDM / psi(z) */
    if (pba->octh.is_enabled) {
      double z = 1.0/a - 1.0;  /* redshift from scale factor */
      double psi = octh_psi(&(pba->octh), z);
      pvecback[pba->index_bg_H] = H_lcdm / psi;
    } else {
      pvecback[pba->index_bg_H] = H_lcdm;
    }
  }

    /* H' with exact OCTH derivative */
  {
    double H_prime_lcdm = - (3./2.) * (rho_tot + p_tot) * a + pba->K/a;
    if (pba->octh.is_enabled) {
      double z = 1.0/a - 1.0;
      double H_lcdm = sqrt(rho_tot-pba->K/a/a);
      pvecback[pba->index_bg_H_prime] = octh_modify_H_prime(&(pba->octh), 
                                                             H_lcdm, H_prime_lcdm,
                                                             pvecback[pba->index_bg_H], z);
    } else {
      pvecback[pba->index_bg_H_prime] = H_prime_lcdm;
    }
  }
  ...
  /* For OCTH: rho_crit = H^2, where H has been modified by OCTH */
  /* This ensures consistency between H and rho_crit */
  rho_crit = pvecback[pba->index_bg_H] * pvecback[pba->index_bg_H];
