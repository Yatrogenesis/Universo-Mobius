...
int input_read_parameters_general(struct file_content * pfc,
  ...
  /** 11) OCTH (Observational Cosmological Topological Hypothesis) parameters */
  {
    int octh_flag = 0;
    class_call(parser_read_string(pfc,"octh",&string1,&flag1,errmsg),errmsg,errmsg);
    if (flag1 == _TRUE_) {
      if (strcmp(string1,"off") == 0 || strcmp(string1,"no") == 0) {
        pba->octh.is_enabled = 0;
        pba->octh.profile = octh_off;
      }
      else if (strcmp(string1,"gaussian") == 0 || strcmp(string1,"yes") == 0) {
        pba->octh.is_enabled = 1;
        pba->octh.profile = octh_gaussian;
        octh_flag = 1;
      }
      else if (strcmp(string1,"extended") == 0) {
        pba->octh.is_enabled = 1;
        pba->octh.profile = octh_extended;
        octh_flag = 1;
      }
      else if (strcmp(string1,"pit") == 0) {
        pba->octh.is_enabled = 1;
        pba->octh.profile = octh_pit;
        octh_flag = 1;
      }

    }
    
    /* Read OCTH parameters if enabled */
    if (octh_flag) {
      class_read_double("octh_amplitude",pba->octh.amplitude);
      class_read_double("octh_z_topo",pba->octh.z_topo);
      class_read_double("octh_sigma",pba->octh.sigma);
      
      if (pba->octh.profile == octh_extended) {
        class_read_double("octh_eps_peak",pba->octh.eps_peak);
        class_read_double("octh_eps_tail",pba->octh.eps_tail);
        class_read_double("octh_z_tail",pba->octh.z_tail_start);
        class_read_double("octh_tail_width",pba->octh.z_tail_width);
      }
      if (pba->octh.profile == octh_pit) {
        class_read_double("octh_epsilon_topo",pba->octh.epsilon_topo);
        class_read_double("octh_z_rec",pba->octh.z_rec);
        class_read_double("octh_sigma_rec",pba->octh.sigma_rec);
      }
      
      /* OCTH verbose output removed for compilation */
    }
  }
...
int input_default_params(struct background *pba,
  ...
  /** 11) OCTH (Observational Cosmological Topological Hypothesis) defaults */
  octh_init_defaults(&(pba->octh));
